"""
Flask app assembly (Milestone 8) -- legacy synchronous HTTP-polling dashboard.
...
Status (real-time streaming refactor)
--------------------------------------
`astra.dashboard.server` is now a FastAPI + WebSocket app (real-time
mock telemetry streaming, `/ws/telemetry`) -- see that module's
docstring. This module (unchanged apart from this note) still owns the
Scenario Builder page/routes (`astra.dashboard.scenario_routes`) and
the geo-layer map shell (`index.html`, `astra/dashboard/geo/*.json`),
neither of which have been ported to FastAPI: both are mid-redesign per
`docs/dashboard_redesign_progress.md` ("no backend algorithm/route
changes outside what's explicitly logged there"), so porting them was
deliberately left out of this refactor's scope rather than risking
breaking that in-progress work.

`astra.dashboard.server` mounts this module's Flask `app` (via
`a2wsgi.WSGIMiddleware`) at `/`, so `create_app()`'s routes -- `/`,
`/state`, `/scenario*` -- are all still reachable exactly as before,
just served through the new FastAPI process instead of a bare `Flask`
dev server. `create_app()`/`run_dashboard_in_background()` themselves
are unchanged and remain directly usable (e.g. for live-BlueSky mode,
which does not need real-time WebSocket streaming and is out of scope
for this refactor -- see `main.py`).
"""
