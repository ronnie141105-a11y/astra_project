...
def test_serialize_sector_regions(r: Runner) -> None:
    """Milestone 9: `{sector_name: ComplexityRegion}` serializes like any other region."""
    sector_regions = {"GVA": _region(["A1", "A2"], 62.0)}
    out = serializers.serialize_sector_regions(sector_regions)
    r.check("sector key present", "GVA" in out)
    r.check_close("sector complexity_score round-trips", out["GVA"]["complexity_score"], 62.0)


def test_serialize_sector_history(r: Runner) -> None:
    """Milestone 9: rolling per-sector samples serialize oldest-first."""
    from astra.complexity.sector import SectorComplexitySample

    sector_history = {
        "GVA": [
            SectorComplexitySample(bucket_start_s=0.0, complexity_score=20.0, aircraft_count=2),
            SectorComplexitySample(bucket_start_s=300.0, complexity_score=45.0, aircraft_count=4),
        ]
    }
    out = serializers.serialize_sector_history(sector_history)
    r.check("two samples for GVA", len(out["GVA"]) == 2)
    r.check_close("oldest bucket first", out["GVA"][0]["bucket_start_s"], 0.0)
    r.check("aircraft_count round-trips", out["GVA"][1]["aircraft_count"] == 4)
...
def test_serialize_resolution_candidate_with_components_and_path(r: Runner) -> None:
    """Milestone 9: before/after component bars and the what-if path serialize."""
    before_components = {"density_ac_per_nm2": 0.02, "mtca_count": 1.0}
    after_components = {"density_ac_per_nm2": 0.01, "mtca_count": 0.0}
    prediction = _prediction(source_time_s=0.0, horizons=(5, 10))
    candidate = _candidate(
        target="A1",
    ...
    out = serializers.serialize_resolution_candidate(candidate)
    r.check(
        "before components round-trip",
        out["complexity_before_components"] == before_components,
    )
    r.check(
        "after components round-trip",
        out["complexity_after_components"] == after_components,
    )
    r.check(
        "hypothetical path has one point per horizon",
        len(out["hypothetical_path"]) == 2,
    )
    r.check(
        "hypothetical path ordered by ascending horizon",
        [p["horizon_min"] for p in out["hypothetical_path"]] == [5, 10],
    )
...
def main() -> None:
    ...
    test_serialize_sector_regions(r)
